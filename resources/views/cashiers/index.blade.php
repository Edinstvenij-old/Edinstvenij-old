            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 mt-4">
                        ...
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Photo') }}</th>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Total Exchanges') }}</th>
                        ...
                        @foreach($cashiers as $cashier)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">{{ $cashier->phone }}</td>
                                    ...
                                    <a href="{{ route('cashiers.edit', $cashier->id) }}"
                                       class="inline-flex items-center px-3 py-1 rounded-md bg-green-600 text-white ring-1 ring-green-700 hover:bg-green-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-transform transform hover:scale-105 duration-300 ease-in-out">
                                        <svg class="w-4 h-4 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 6.343a8 8 0 00-11.314 11.314l-2.828 2.828a1 1 0 00-.293.707V20a1 1 0 001 1h1.586a1 1 0 00.707-.293l2.828-2.828a8 8 0 0011.314-11.314l-2.828-2.828zM7 13l4-4m0 4L3 8m12 0l2 2m-2-2l4 4" />
                                        </svg>
                                        {{ __('Edit') }}
                                    </a>
                                    <!-- Delete Button -->
                                    <form action="{{ route('cashiers.destroy', $cashier->id) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure you want to delete this cashier?') }}');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
